...
describe('purchase-requests.service', () => {
  ...
  it('mapBackendPRToFrontend correctly maps backend fields including new schema names', () => {
    const rawPR = {
      ...
      items: [
        { id: 1, item_name: 'Monitor', quantity: 2, unit_price: 1250.25, total_price: 2500.50 }
      ],
      attachments: []
    };
  ...
  it('getPurchaseRequests fetches and maps list of PRs', async () => {
    (global.fetch as any).mockResolvedValueOnce({
      ...
      json: async () => ({
        purchase_requests: [
          { id: 1, pr_number: 'PR-1', total_estimated_cost: '100', purpose: 'Test' }
        ]
      })
    });

    const prs = await getPurchaseRequests('Test', 'Eng', 'Submitted');
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('search=Test'), expect.any(Object));
  ...
  it('createPurchaseRequest sends POST request and formats default line item if empty', async () => {
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        purchase_request: { id: 2, pr_number: 'PR-2', total_estimated_cost: '500', purpose: 'Office supplies' }
      })
    });

    const created = await createPurchaseRequest({ description: 'Office supplies', amount: 500 });
    expect(global.fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/api/purchase-requests', {
      method: 'POST',
  ...
  it('updatePurchaseRequest and deletePurchaseRequest perform API calls correctly', async () => {
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ purchase_request: { id: 2, pr_number: 'PR-2', purpose: 'Updated' } })
    });

    const updated = await updatePurchaseRequest('2', { description: 'Updated', amount: 500 });
    ...
    expect(global.fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/api/purchase-requests/2', expect.objectContaining({ method: 'DELETE' }));
  });
  ...
  it('bulkDeletePurchaseRequests posts IDs array', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('http://127.0.0.1:8000/api/purchase-requests/bulk-delete', {
      method: 'POST',
      headers: expect.any(Object),
