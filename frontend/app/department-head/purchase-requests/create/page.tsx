...
export default function CreatePRPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!purpose.trim() || !category || items.some((i) => !i.itemName.trim())) return;

    if (categoryBudget && totalAmount > categoryBudget.available) {
      alert(`Insufficient budget. Available: ₱${categoryBudget.available.toLocaleString()}, Requested: ₱${totalAmount.toLocaleString()}`);
      return;
    }
      ...
      <div className="max-w-xl mx-auto mt-16 text-center">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-12">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { setSubmitted(false); setPurpose(""); setCategory(""); setItems([defaultItem()]); setAttachments([]); }}
            ...
            <button
              onClick={() => router.push("/department-head/purchase-requests")}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-semibold text-secondary mb-1.5">Purpose / Justification <span className="text-red-500">*</span></label>
              <textarea
                value={purpose}
          ...
          {categoryBudget && (
            <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
              <p className="text-sm font-semibold text-secondary mb-2">Category Budget Information</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div>
                  <p className="text-secondary/60 text-xs">Allocated</p>
                  <p className="font-semibold text-secondary">₱{categoryBudget.allocated.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-secondary/60 text-xs">Reserved</p>
                  <p className="font-semibold text-amber-600">₱{categoryBudget.reserved.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-secondary/60 text-xs">Spent</p>
                  <p className="font-semibold text-blue-600">₱{categoryBudget.spent.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-secondary/60 text-xs">Available</p>
                  <p className="font-semibold text-emerald-600">₱{categoryBudget.available.toLocaleString()}</p>
                </div>
              </div>
              {totalAmount > categoryBudget.available && (
                <div className="mt-3 p-2 bg-red-50 rounded-lg border border-red-200">
                  <p className="text-xs text-red-600 font-semibold">
                    ⚠️ Insufficient budget. Requested: ₱{totalAmount.toLocaleString()}, Available: ₱{categoryBudget.available.toLocaleString()}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            disabled={submitting || !!(categoryBudget && totalAmount > categoryBudget.available)}
