...
export default function CompanyBudgetPage() {
  ...
  const [activeCompanyBudget, setActiveCompanyBudget] =
    useState<CompanyBudget | null>(null);
  ...
  const [viewingBreakdownDept, setViewingBreakdownDept] =
    useState<DeptSummaryWithBreakdown | null>(null);

  // Company Budget modal state
  const [showCompanyBudgetModal, setShowCompanyBudgetModal] = useState(false);
  const [companyBudget, setCompanyBudget] = useState<CompanyBudget | null>(
    null,
  );
  ...
  const [cbSummary, setCbSummary] = useState<BudgetSummaryResponse | null>(
    null,
  );
  ...
  const loadCbModalData = async (targetYear: number) => {
    ...
    setCbAmount(
      existing && Number(existing.total_budget) > 0
        ? String(existing.total_budget)
        : "",
    );
    ...
    if (prevCb && prevSummary) {
      prevRem =
        Number(prevCb.total_budget || 0) +
        Number(prevCb.carry_forward || 0) -
        Number(prevSummary.total_reserved || 0) -
        Number(prevSummary.total_spent || 0);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Admin" },
          { label: "Company Budget Management" },
        ]}
        actions={
          <div className="flex items-center gap-2">
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 00-2 2h2m2 4h10a2 2 0 002-2v-4H7v4a2 2 0 002 2zM9 9h6v6H9V9z"
                  />
                </svg>
                Print Report
              </button>
            ...
            <button
              ...
              onClick={async () => {
                ...
                const futureExistingYears = Array.from(
                  new Set(existingYears.filter((y) => y >= currentYear)),
                );
                ...
                const targetYear = futureExistingYears.includes(selectedYear)
                  ? selectedYear
                  : futureExistingYears[0];
      ...
      {loading ? (
        ...
        <div className="p-12 text-center text-secondary/50">
          No budget data found for FY {fiscalYear}. Click{" "}
          <strong>Edit Budget</strong> on any department to set allocations.
        </div>
      ) : (
        <>
          {/* Stats */}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {summary.department_summaries.map((dept) => {
                      const utilization =
                        dept.allocated > 0
                          ? ((dept.reserved + dept.spent) / dept.allocated) *
                            100
                          : 0;
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-3.5 font-semibold text-secondary">
                            {dept.allocated > 0 ? (
                              `₱${safeNum(dept.allocated).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                            ) : (
                              <span className="text-secondary/40">₱0.00</span>
                            )}
                          </td>
                          <td className="px-6 py-3.5">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary">
                              {(safeNum(activeCompanyBudget?.total_budget) +
                                safeNum(activeCompanyBudget?.carry_forward) >
                              0
                                ? (safeNum(dept.allocated) /
                                    (safeNum(
                                      activeCompanyBudget?.total_budget,
                                    ) +
                                      safeNum(
                                        activeCompanyBudget?.carry_forward,
                                      ))) *
                                  100
                                : 0
                              ).toFixed(2)}
                          ...
                          <td className="px-6 py-3.5 text-amber-600 font-semibold">
                            ₱
                            {safeNum(dept.reserved).toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
                          <td className="px-6 py-3.5 text-red-600 font-semibold">
                            ₱
                            {safeNum(dept.spent).toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
                          <td className="px-6 py-3.5 text-emerald-600 font-semibold">
                            ₱
                            {safeNum(dept.available).toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-slate-200 bg-slate-50">
                      <td className="px-6 py-3.5 font-bold text-secondary">
                        Total
                      </td>
                      <td className="px-6 py-3.5 font-bold text-secondary">
                        ₱
                        {summary.total_allocated.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      ...
                      <td className="px-6 py-3.5 font-bold text-amber-600">
                        ₱
                        {summary.total_reserved.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-3.5 font-bold text-red-600">
                        ₱
                        {summary.total_spent.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-3.5 font-bold text-emerald-600">
                        ₱
                        {summary.total_available.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-primary/5 rounded-xl p-3 border border-primary/10">
                  <span className="text-xs text-secondary/60 font-semibold">
                    Total Year Allocated
                  </span>
                  <p className="text-base font-bold text-primary">
                    ₱{viewingBreakdownDept.allocated.toLocaleString()}
                  </p>
                </div>
                <div className="bg-amber-50 rounded-xl p-3 border border-amber-100">
                  ...
                  <p className="text-base font-bold text-amber-700">
                    ₱{viewingBreakdownDept.reserved.toLocaleString()}
                  </p>
                </div>
                <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100">
                  <span className="text-xs text-emerald-800/60 font-semibold">
                    Total Year Available
                  </span>
                  <p className="text-base font-bold text-emerald-700">
                    ₱{viewingBreakdownDept.available.toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(viewingBreakdownDept.monthly_breakdown || []).map(
                      (mb) => {
                        const mUtil =
                          mb.allocated > 0
                            ? ((mb.reserved + mb.spent) / mb.allocated) * 100
                            : 0;
                        return (
                          <tr
                            key={mb.month}
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right font-semibold text-secondary">
                              ₱{mb.allocated.toLocaleString()}
                            </td>
                            <td className="px-4 py-3 text-right font-medium text-amber-600">
                              ₱{mb.reserved.toLocaleString()}
                            </td>
                            <td className="px-4 py-3 text-right font-medium text-blue-600">
                              ₱{mb.spent.toLocaleString()}
                            </td>
                            <td className="px-4 py-3 text-right font-semibold text-emerald-600">
                              ₱{mb.available.toLocaleString()}
                            </td>
            ...
            <div className="flex shrink-0 items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/90 backdrop-blur-md rounded-t-2xl z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-secondary/60">
                    Set the overall company budget for a fiscal year
                  </p>
                </div>
              </div>
              <button
            ...
            <>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  if (isNaN(amt) || amt < 0) {
                    setCbError(
                      "Please enter a valid positive amount for Approved Budget.",
                    );
                    return;
                  }
                  if (isNaN(cf) || cf < 0) {
                    setCbError(
                      "Please enter a valid positive amount for Carry Forward.",
                    );
                    return;
                  }
                  if (cf > cbPrevRemaining) {
                    setCbError(
                      `Carry forward cannot exceed previous year's remaining balance (₱${cbPrevRemaining.toLocaleString()}).`,
                    );
                    return;
                  }
                  ...
                  setCbSuccess("");
                  try {
                    const saved = await upsertCompanyBudget(
                      cbFiscalYear,
                      amt,
                      cf,
                    );
                ...
                <div className="p-6 space-y-5 overflow-y-auto">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-xs font-bold uppercase tracking-wider text-secondary/50">
                        Fiscal Year
                      </label>
                      <button
                        type="button"
                          ...
                          <button
                            ...
                            className={`shrink-0 px-2 py-1 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${
                              year === cbFiscalYear
                                ...
                                : "text-secondary/60 hover:text-secondary hover:bg-white"
                            }`}
                          >
                            {year}
                          </button>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-secondary/50 mb-2">
                      FY {cbFiscalYear - 1} Remaining Budget
                    </label>
                    ...
                    <p className="text-xs text-secondary/50 mt-1">
                      Remaining budget automatically calculated from previous
                      fiscal year.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-secondary/50 font-semibold text-sm">
                        ₱
                      </span>
                      <input
                        type="number"
                  ...
                  <div>
                    ...
                    <p className="text-xs text-secondary/50 mt-1">
                      You may reduce or set this to zero, but it cannot exceed
                  ...
                  <div>
                    ...
                    <div className="px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 font-extrabold text-lg flex items-center justify-between">
                      <span>
                        ₱
                        {(
                          Number(cbAmount || 0) + Number(cbCarryForward || 0)
                        ).toLocaleString()}
                      </span>
                      <span className="text-xs font-semibold text-emerald-700/60 uppercase">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-secondary/70 mb-3">
                      Fiscal Summary Status
                    </h4>
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div>
                        <p className="text-xs text-secondary/50 font-semibold mb-1">
                          Reserved Amount
                        </p>
                        <p className="font-bold text-gold">
                          ₱{cbSummary?.total_reserved.toLocaleString() || "0"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-secondary/50 font-semibold mb-1">
                        ...
                        <p className="font-bold text-red-600">
                          ₱{cbSummary?.total_spent.toLocaleString() || "0"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-secondary/50 font-semibold mb-1">
                        ...
                        <p className="font-bold text-accent">
                          ...
                          {(
                            Number(cbAmount || 0) +
                            Number(cbCarryForward || 0) -
                            (cbSummary?.total_reserved || 0) -
                            (cbSummary?.total_spent || 0)
                          ).toLocaleString()}
                        </p>
                      </div>
                    </div>
