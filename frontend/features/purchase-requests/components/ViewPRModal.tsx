...
export default function ViewPRModal({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const updatedPR = await uploadPRAttachments(
        pr.id,
  ...
  const handleDeleteAttachment = async (attId: number) => {
    if (!confirm("Are you sure you want to delete this attachment?") || !pr)
      return;
    try {
        ...
        <div className="p-6 space-y-6">
          ...
          {pr.status !== "Rejected" && (
            <div>
              ...
              <div className="flex flex-wrap items-center w-full gap-2 bg-slate-50 p-4 rounded-xl border border-slate-100">
                {[
                  "Draft",
                  "Pending",
                  "Approved",
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex items-center justify-center px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${isCurrent ? "bg-primary text-white shadow-md ring-2 ring-primary/20" : isCompleted ? "bg-primary/20 text-primary" : "bg-slate-200 text-slate-400"}`}
                      >
                        {stage}
                      </div>
                      {idx < arr.length - 1 && (
                        <div
                          className={`w-4 h-0.5 rounded-full ${isCompleted && !isCurrent ? "bg-primary/50" : "bg-slate-200"}`}
                        />
                      )}
                    </div>
                  );
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden mt-4">
              ...
              <div className="p-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-secondary">
                    {pr.orderer
                      ? `${pr.orderer.first_name} ${pr.orderer.last_name}`
                      : "Not specified"}
                  </p>
                </div>
                <div className="col-span-2">
                  ...
                  <p className="text-sm font-medium text-secondary">
                    {pr.orderedAt
                      ? new Date(pr.orderedAt).toLocaleString("en-US", {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                          hour: "numeric",
                ...
                localAttachments.map((att: Attachment) => (
                  ...
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      att.file_type?.startsWith("video/") ? (
                        <div
                          ...
                          onClick={() =>
                            setPreviewImage(att.preview_url || null)
                          }
                      ...
                      <div className="truncate">
                        ...
                        <p className="text-[10px] text-slate-400">
                          {(att.file_size / 1024).toFixed(1)} KB • Uploaded by{" "}
                          {att.uploader
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        onClick={() =>
                          downloadPRAttachment(pr.id, att.id, att.file_name)
                        }
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                      <div>
                        ...
                        <p className="text-sm font-medium text-secondary">
                          {hist.changer
                            ? `${hist.changer.first_name} ${hist.changer.last_name}`
                            : "System User"}
                        </p>
                      </div>
                      <div>
